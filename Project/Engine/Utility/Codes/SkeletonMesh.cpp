...
void Engine::SkeletonMesh::ChangeModel(
	...
	const std::filesystem::path& FileFullPath)&
{
	...
	//	const uint32 CurEndAnimIdx = AnimInfoTable.size() - 1u;

	//	for (uint32 AnimIdx = 0u; AnimIdx < AiScene->mNumAnimations; ++AnimIdx)
	//	{
	//		aiAnimation* _Animation = AiScene->mAnimations[AnimIdx];
	//		AnimationInformation CurPushAnimInfo{};

	//		const uint32 CurPushAnimIdx = CurEndAnimIdx + AnimIdx;

	//		CurPushAnimInfo.Acceleration = 1.0 * _Animation->mTicksPerSecond;
	//		CurPushAnimInfo.TickPerSecond = _Animation->mTicksPerSecond;
	//		CurPushAnimInfo.Name = (_Animation->mName.C_Str());
	//		CurPushAnimInfo.Duration = _Animation->mDuration;

	//		AnimInfoTable.push_back(CurPushAnimInfo);
	//		AnimIdxFromName.insert({ _Animation->mName.C_Str() , CurPushAnimIdx });
	...
	//			{
	//				// 인덱스 . 
	//				_AnimationTrack->ScaleTimeLine[CurPushAnimIdx][ChannelName]
	//					[_Animation->mChannels[ChannelIdx]->mScalingKeys[ScaleKeyIdx].mTime] 
	//				= FromAssimp(_Animation->mChannels[ChannelIdx]->mScalingKeys[ScaleKeyIdx].mValue);
	//			}
...
void Engine::SkeletonMesh::AddModel(IDirect3DDevice9* const Device, 
	const std::filesystem::path& FileFullPath)&
{
	using VertexType = Vertex::LocationTangentUV2DSkinning;
	...
	{
		const std::wstring CurrentResourceName =
			FileFullPath.wstring()+ L"_" + std::to_wstring(SkeletonResourceID++);
		...
		const std::wstring MeshVtxBufResourceName =
			FileFullPath.wstring() + L"SkeletonMesh_VertexBuffer_" + CurrentResourceName;
		IDirect3DVertexBuffer9* _VertexBuffer{ nullptr };
		CreateMesh.VtxBufSize = sizeof(VertexType) * Verticies->size();
		Device->CreateVertexBuffer(CreateMesh.VtxBufSize, D3DUSAGE_DYNAMIC, NULL,
			D3DPOOL_DEFAULT, &_VertexBuffer, nullptr);
		CreateMesh.VertexBuffer = ResourceSys.Insert<IDirect3DVertexBuffer9>(
			MeshVtxBufResourceName, _VertexBuffer);
		CreateMesh.PrimitiveCount = CreateMesh.FaceCount = _AiMesh->mNumFaces;
		...
		const std::wstring MeshIdxBufResourceName =
			FileFullPath.wstring() +L"SkeletonMesh_IndexBuffer_" + CurrentResourceName;
		IDirect3DIndexBuffer9* _IndexBuffer{ nullptr };
		const uint32 IdxBufSize = sizeof(uint32) * Indicies.size();
		Device->CreateIndexBuffer(IdxBufSize, D3DUSAGE_WRITEONLY, D3DFMT_INDEX32,
			D3DPOOL_MANAGED, &_IndexBuffer, nullptr);
	...
	if (AiScene->HasAnimations())
	{
		...
		for (uint32 AnimIdx = 0u; AnimIdx < AiScene->mNumAnimations; ++AnimIdx)
		{
			...
			std::unordered_map<std::string,
				std::map<double, Vector3>> CurScaleTimeLine{};

			std::unordered_map<std::string,
				std::map<double, Quaternion>> CurQuatTimeLine{};

			std::unordered_map<std::string,
				std::map<double, Vector3>> CurPosTimeLine{};

			CurAnimInfo.Acceleration = 1.0 * _Animation->mTicksPerSecond;
			CurAnimInfo.TickPerSecond = _Animation->mTicksPerSecond;
			CurAnimInfo.Name = (_Animation->mName.C_Str());
			CurAnimInfo.Duration = _Animation->mDuration;

			AnimInfoTable.push_back(CurAnimInfo);
			const uint32 CurAnimInfoIdx = AnimInfoTable.size() - 1u;

			AnimIdxFromName.insert({ _Animation->mName.C_Str() , CurAnimInfoIdx });
			for (uint32 ChannelIdx = 0u; ChannelIdx < _Animation->mNumChannels; ++ChannelIdx)
			{
				const std::string ChannelName = _Animation->mChannels[ChannelIdx]->mNodeName.C_Str();
				...
				for (uint32 ScaleKeyIdx = 0u;
					...
					++ScaleKeyIdx)
				{
					CurScaleTimeLine[ChannelName][_Animation->mChannels[ChannelIdx]->mScalingKeys[ScaleKeyIdx].mTime]
						= FromAssimp(_Animation->mChannels[ChannelIdx]->mScalingKeys[ScaleKeyIdx].mValue); ;
				}
					...
					++RotationKeyIdx)
				{
					...
					CurQuatTimeLine[ChannelName][_Animation->mChannels[ChannelIdx]->mRotationKeys[RotationKeyIdx].mTime]
						= _Quat;
				}
					...
					++PosKeyIdx)
				{
					...
					CurPosTimeLine[ChannelName][_Animation->mChannels[ChannelIdx]->mPositionKeys[PosKeyIdx].mTime] 
						= FromAssimp(_Animation->mChannels[ChannelIdx]->mPositionKeys[PosKeyIdx].mValue);
				}
			}


			_AnimationTrack->ScaleTimeLine.push_back(CurScaleTimeLine);
			_AnimationTrack->QuatTimeLine.push_back(CurQuatTimeLine);
			_AnimationTrack->PosTimeLine.push_back(CurPosTimeLine);
...
void Engine::SkeletonMesh::Event(Object* Owner)&
{
	...
	if (Engine::Global::bDebugMode)
	{
		if (ImGui::TreeNode(("SkeletonEdit_" + ToA(Owner->GetName())).c_str()))
		{
			...
			//auto DebugPrintMatrixImGui= [](const Matrix& _Matrix) 
			//{
			//	for (uint32 Row = 0u; Row < 4u; ++Row)
			//	{
			//		for (uint32 Col = 0u; Col < 4u; ++Col)
			//		{
			//			ImGui::Text("%.3f,", _Matrix(Row, Col));
			...
			//	// 	std::cout << std::endl;
			//};
			//for (const auto& _Bone : BoneTable)
			//{
			//	ImGui::Text("Name : %s", _Bone->Name.c_str());
			//	ImGui::Separator();

			//	ImGui::Text("Final");
			//	DebugPrintMatrixImGui(_Bone->Final);
			//	ImGui::Separator();

			//	_Bone->Offset;
			//	_Bone->OriginTransform;
			//	_Bone->Parent;
			//	_Bone->Transform;
			//	_Bone->ToRoot;
			//}

			/*for (const auto& RenderMatrix : RenderBoneMatricies)
			{
				FMath::DebugPrintMatrixImGui(RenderMatrix);
			}*/
		}
	}
}
